...
graph TD
    ...
    subgraph VaultTier ["Central Orchestration Tier (The Vault)"]
        ...
        Cooling["Active Cooling (Dual-Fan System)"]:::vaultStyle
        VaultPower["USB-C PSU / Power Bank"]:::vaultStyle
    ...
    subgraph MobilePOS ["Mobile Integration Tier"]
        Phone["Mobile Device (Mesh-POS Client Interface)"]:::vaultStyle
    end

    %% Mobile connection
    Phone == "Local Wi-Fi / Bluetooth" ==> Vault
...
*   **System Storage**: A 128GB high-endurance Class 10 UHS-I MicroSD Card. This capacity is necessary to support a local-first time-series database containing extensive historical telemetry, local transaction ledgers, intrusion alert logs captured by the security gateway, and system log files.
*   **Peripherals**: 
    *   A low-power portable projector. This peripheral enables local administrators to manage system configurations, run diagnostic checks, and present analytical data (such as soil moisture analytics and irrigation/watering alerts) directly to community groups in field conditions where traditional monitors are impractical.
*   **Thermal Management**: An active dual-fan cooling system (copper micro-heatsinks have been omitted to minimize deployment costs). This active system is required to mitigate the high ambient temperatures characteristic of Bulawayo and Tsholotsho, preventing thermal throttling and maintaining processing throughput.
*   **Power Supply**: Powered via a standard 5V USB-C Power Supply Unit (PSU) or a portable power bank, providing operational flexibility during grid disruptions.
...
*   **Edge-Alert Security Gateway (Perimeter Security)**:
    *   *Passive Infrared (PIR) Sensors (HC-SR501)*: Low-power infrared motion detectors with adjustable sensitivity and delay timers. These function as physical triggers connected to the Pico W, which send state updates (alerts) to the Orchestrator via MQTT.
    *   *Ultrasonic Distance Sensors (HC-SR04)*: Capable of non-contact distance measurement from 2cm to 400cm (±3mm accuracy), used to construct virtual tripwires connected to the Pico W to detect physical entry, publishing state alerts to the central hub.
...
#### 4. Mobile Point-of-Sale (POS) Client Tier
To eliminate the cost and complexity of dedicated scanner and reader modules on every node, Point-of-Sale (POS) client terminals are offloaded to consumer-grade mobile devices (smartphones or tablets) operated by merchants or community leads.
*   **NFC/RFID Token Reading**: Built-in native NFC transceiver chip on guest mobile devices (operating at 13.56 MHz) is utilized to scan customer card badges, offline payment tokens, or local store credits.
*   **Barcode/QR Scanning**: Integrated mobile camera utilizing an offline web-based or native barcode decoding library (e.g., HTML5-QR-Scanner) to parse inventory barcodes and digital wallet QR screens.
*   **Vault Connectivity Transceiver**: Native Wi-Fi (802.11n) and/or Bluetooth (RFCOMM/SPP profiles) transceivers to connect directly to the Central Orchestrator's local gateway for local offline database synchronization and checkout execution.

#### 5. Power & Enclosure Subsystem
...
| :--- | :--- | :--- | :--- |
| **Operating System** | Kali Linux (ARM64 headless) | Base operating system for orchestrator | Pi 4 Hub |
...
| **Machine Learning**| TensorFlow Lite Runtime | Local irrigation/watering requirement inference | Pi 4 Hub |
| **Transaction Storage**| SQLite | Resilient, local transaction ledger | Pi 4 Hub |
| **Web-POS Client** | HTML5 / Vanilla JS | Client interface for scanning QR/NFC transactions | Mobile Device |
| **POS Service Daemon** | Python SocketServer / PyBluez | Processes socket/RFCOMM checkout requests | Pi 4 Hub |
| **CAD & Modeling** | FreeCAD & KiCAD | Enclosure and PCB design toolchain | Workstation |

#### 1. Operating Systems & System Firmware
*   **Orchestrator Platform**: Kali Linux (ARM64 headless, Debian-based core). The headless minimal version of Kali Linux is mandatory to conserve CPU cycles and system memory, reserving maximum resources for the system services and the TensorFlow Lite inference engine.
...
*   **Edge-Alert Security Gateway Core**: A custom Python event-handler script. Instead of running complex computer vision libraries, the orchestrator script on the Pi 4 listens for sensor-trigger MQTT messages from the Pico W nodes. Upon receiving a trigger, the script logs the intrusion event to a local database and dispatches immediate state-change alerts across the local network to notify administrators.
*   **Agri-Analytics Inference**: TensorFlow Lite (TF Lite) Interpreter runtime. The model is built using TensorFlow and converted to a quantized flatbuffer format (`.tflite`) to execute low-latency predictive analysis on localized soil moisture and watering requirements using the Pi 4 CPU cores without network overhead.
*   **Mesh-POS Transaction Ledger**: SQLite database engine embedded within a Python-based server. The server exposes a WebSocket and local TCP/RFCOMM socket API to process incoming checkout transactions from guest mobile devices. SQLite's local-first transactional integrity (ACID compliance) guarantees that sales records, inventory shifts, and client-scanned NFC token records are safely written to the MicroSD card, prepared for mesh synchronization once connected to adjacent shop hubs.
...
*   **KiCAD (v7.0+)**: Open-source schematic capture and PCB layout editor used to design custom hardware shields, routing I/O from the Pico W directly to the swappable sensor and interface module connections.
*   **Blender (v3.6+)**: 3D computer graphics software used for modeling spatial distributions and simulating sensor coverage fields of the Edge-Alert Security Gateway in rural and farm settings.
